...
def si_barrier_cert(dxi, x, obstacles, barrier_gain=100, safety_radius=0.3, obs_safety=0.4, magnitude_limit=0.25):
    start_time = time.time()
    N = dxi.shape[1]
    #print(N)
    #print(obstacles.shape[1])
    num_constraints = int(comb(N, 2))
    A = np.zeros((num_constraints, 2*N))
    b = np.zeros(num_constraints)
    if obstacles.any():
        Aob = np.zeros((N*obstacles.shape[1], 2*N))
        bob = np.zeros(N*obstacles.shape[1])
    ...
    if (obstacles.any()):
        for i in range(N):
            for k in range(obstacles.shape[1]): #Compare to the another compared agent
                errorObs = x[:, i] - obstacles[:, k] #Distance between an agent and an obstacle
                # print(errorObs)

                hObs = errorObs[0]*errorObs[0] + errorObs[1]*errorObs[1] - np.power(obs_safety, 2)

                # data4.append(errorObsScalar) 
                # print(math.sqrt((errorObs[0]*errorObs[0] + errorObs[1]*errorObs[1] + errorObs[2]*errorObs[2])))
                # print(hObs)
                Aob[countObstacles, (2*i, (2*i+1))] = -2*errorObs
