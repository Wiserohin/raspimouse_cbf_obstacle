...
def si_barrier_cert(dxi, x, obstacles, barrier_gain=100, safety_radius=0.3, magnitude_limit=0.25):
    ...
    if (obstacles.any()):
        for i in range(N):
            for k in range(obstacles.shape[1]): #Compare to the another compared agent
                ...
                hObs = errorObs[0]*errorObs[0] + errorObs[1]*errorObs[1] - np.power(safety_radius, 2)
