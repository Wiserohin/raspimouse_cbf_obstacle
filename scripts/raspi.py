...
def get_lidar_values(N):
    for i in range(N):
        rot_angle = rospy.wait_for_message('/raspi_'+str(i)+'/odom', Odometry)
        rot_angle = np.arccos(rot_angle.pose.pose.orientation.w) * 2
        scan = rospy.wait_for_message("/raspi_{}/scan".format(str(i)), LaserScan)
        scan = scan.ranges
        # print(scan)
        closest_distance = np.min(scan)
        if np.isinf(closest_distance):
            return None
        closest_angle = np.deg2rad(np.argmin(scan) + 1 + 180) # lidar seems to face the opposite way for some reason
